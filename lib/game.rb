class Game
  ...
  def next_room #Метод генерации следующей комнаты
    ...
    case @room_type
      when 1..30 then
        monster_room #Выводим случайную комнату с монстром
      when 31..60 then
        riddle_room #Выводим случайную комнату с загадкой
      when 61..70 then
        empty_room #Выводим пустую комнату
      when 71..100 then
        math_room #Выводим комнату со случайным примером
    end
